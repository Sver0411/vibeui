...
│   ├── collections.ts      # 合集定义
│   ├── metas.ts            # 资源瘦元数据汇总
│   ├── client.ts           # 客户端安全元数据与搜索词表
│   ├── index.ts            # 服务端完整元数据与 AI 提示词
...
**第 2 步**：在 `src/registry/metas.ts` 中 import 并加入 `RAW_RESOURCES` 数组。若提供自定义 AI 提示词，在资源目录中新增 `ai.ts` 并接入 `ai-data.ts`；供客户端搜索使用的术语同步到 `ai-terms.ts`。
...
- **无限循环**：iframe 沙箱不保证执行线程隔离，`while(true)` 仍可能卡住页面；不要运行不可信代码。
